...
static void tcp_server_handle(struct tcp_pcb *tpcb, struct tcp_server_struct *es)
{
    ...
    // Odczytanie danych wejściowych z bufora
    char *input = (char *)es->p->payload;
    size_t input_len = es->p->tot_len;

    if (strncmp(input, "TCP00", input_len) == 0)
    {
        strncpy(response, "Hello World", sizeof(response) - 1);
    }
    else if (strncmp(input, "TCP01", input_len) == 0)
    {
        strncpy(response, "RafalBartoszak", sizeof(response) - 1);
    }
    else
    {
        strncpy(response, "ERR", sizeof(response) - 1);
    }

    // Wysłanie odpowiedzi do klienta
    tcp_write(tpcb, response, strlen(response), TCP_WRITE_FLAG_COPY);

    // Zwalnianie bufora odbiorczego
